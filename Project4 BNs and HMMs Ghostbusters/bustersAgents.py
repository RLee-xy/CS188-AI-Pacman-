...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # find most likely position of ghost
        mostlikelypos = []
        for distribution in livingGhostPositionDistributions:
            #highestkey = max(distribution.values())
            for pos in distribution.items():
                if pos[1] == max(distribution.values()):
                    mostLikely = pos[0]
            mostlikelypos.append(mostLikely)
        ...
        for action in legal:
            ...
            for ghost in mostlikelypos:
                dis = self.distancer.getDistance(ghost, newpos)
                if dis < distance:
                    distance = dis
                    bestact = action
        return bestact
